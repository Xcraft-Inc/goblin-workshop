...
Goblin.registerQuest(goblinName, 'createEntity', function* (
  ...
  if (!id) {
    throw new Error(`workshop.createEntity: missing id`);
  }
  if (!workitemId) {
  ...
  try {
    yield quest.create(id, {
      id,
      desktopId: quest.getSystemDesktop(),
      ...properties,
    });
    yield quest.createFor(workitemId, workitemId, id, {
      id,
      desktopId,
    });
  } finally {
...
Goblin.registerQuest(goblinName, 'trash-entities', function* (
  ...
  yield $(entities)
    ...
    .flatten()
    .compact()
    .map((api) => (n) => api.trashEntity({}, () => n(null, api.id)))
    .nfcall([])
    .parallel(Number.MAX_VALUE)
    .flatten()
    .compact()
