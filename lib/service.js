...
const entityStorageServicePoolSize =
  workshopConfig.entityStorageServicePoolSize || 10;
...
Goblin.registerQuest(goblinName, 'init', function* (quest) {
  ...
  console.log(`UV THREAD POOL SITE: ${process.env.UV_THREADPOOL_SIZE}`);
  setInterval(function () {
    var last = process.hrtime(); // replace Date.now()
    setImmediate(function () {
      var delta = process.hrtime(last); // with process.hrtime()
      if (delta[0] > 0 || delta[1] > 5000000) {
        console.log('///////////////////WARNING/////////////////');
        console.log('       EVENT LOOP LAG:', delta);
        console.log('///////////////////WARNING/////////////////');
      }
    });
  }, 500);
