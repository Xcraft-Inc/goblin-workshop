...
Goblin.registerQuest(goblinName, 'get-mandate-storage-root-path', function*(
  ...
  if (!mandate.storage || !mandate.storage.rootPath) {
  ...
  if (!fs.existsSync(mandate.storage.rootPath)) {
    yield deskAPI.addNotification({
      color: 'red',
      message: `Le stockage est mal configuré, le chemin ${
        mandate.storage.rootPath
      }
      n'existe pas!
  ...
  return mandate.storage.rootPath;
});
