...
Goblin.registerQuest(goblinName, 'init', function* (quest) {
  ...
  //yield quest.cmd(`dragon.init`, {host: 'lab0.epsitec.ch'});
  //yield quest.me.feedDragon({srcDb: 'test'});
  //yield quest.me.findAndCleanOrphans({srcDb: 'test'});
});

Goblin.registerQuest(goblinName, 'findAndCleanOrphans', function* (
  quest,
  srcDb
) {
  const dragon = quest.getAPI('dragon');

  yield dragon.cleanOrphans({
    srcDb,
    fromType: 'document',
  ...
  yield dragon.cleanOrphans({
    srcDb,
    fromType: 'missionOrder',
    toType: 'missionOrderTicket',
  });
});

Goblin.registerQuest(goblinName, 'feed-dragon', function* (quest, srcDb) {
  ...
  for (const type of Object.keys(nonRoot)) {
    yield dragon.feedAggregated({database: srcDb, type});
  }
  ...
  for (const type of rootEntities) {
    yield dragon.feedRoot({database: srcDb, type});
  }
  ...
  for (const [fromType, config] of Object.entries(entityConfiguration)) {
    ...
    if (config.values) {
      for (const [prop, ref] of Object.entries(config.values)) {
        const toType = common.getReferenceType(ref);
        if (common.referenceUseArity(ref)) {
          yield dragon.nestMany({
            database: srcDb,
            parentType: fromType,
            toType: toType,
            property: prop,
