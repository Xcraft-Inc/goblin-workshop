...
// XXX: toto is a stub for testing the state with this singleton goblin
const logicState = {
  toto: true,
};
...
const sendCommand = watt(function*(cmd, data, resp, next) {
  ...
  const unsub0 = _resp.events.subscribe(`${which}::workshop.ripleying`, msg => {
    resp.log.progress('ripleying', msg.data.position, msg.data.length);
  });
  const unsub1 = _resp.events.subscribe(`${which}::workshop.step`, msg => {
...
// XXX: stub to be removed (tetsing stuff)
Goblin.registerQuest(goblinName, 'toto', function(quest) {
  quest.do();
});
