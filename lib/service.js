...
const sendCommand = watt(function* (cmd, data, resp, next) {
  ...
  busClient.once('commands.registry', next.parallel());
...
Goblin.registerQuest(goblinName, 'init', function* (
  ...
  yield quest.cmd(`entity-importer.init`);
  yield quest.cmd(`reindex-entities.init`);
  yield quest.cmd(`rehydrate-entities.init`);
  ...
  const {customIndexesByType, orderIndexesByType} = entityBuilderConfig;
  ...
  let waitNabuInit = null;

  if (quest.hasAPI('nabu')) {
    waitNabuInit = next.parallel();
    try {
      const unsub = quest.sub('*::nabu.initialized', (...args) => {
        unsub();
        waitNabuInit(...args);
      });
...
Goblin.registerQuest(goblinName, 'createEntity', function* (
  ...
  try {
    ...
    const payload = {id: entityId, desktopId};

    if (ttl) {
      payload._goblinTTL = ttl;
    }

    if (view) {
      payload._goblinView = view;
    }

    yield quest.createFor(createFor, createFor, entityId, payload);
  } finally {
...
Goblin.registerQuest(goblinName, 'create-hinter-for', function* (
  ...
  const widgetId = workitemId ? `hinter@${serviceName}@${workitemId}` : null;
  if (!desktopId) {
  ...
  const hinter = yield quest.createFor(goblinName, workitemId, widgetId, {
    id: widgetId,
    name,
    type,
    desktopId,
...
Goblin.registerQuest(goblinName, 'init-storage', function* (
  ...
  if (entityTypes) {
    ...
    for (const entity of entityTypes) {
      const isTransient = configurations[entity].transient || false;
      if (!isTransient) {
...
//init-indexer
//this mapping avoid race init
const initIndexesFlags = {};
Goblin.registerQuest(goblinName, 'init-indexer', function* (
  ...
  if (!indexes) {
    ...
    if (!force && initIndexesFlags[elasticId]) {
      return; //already init
    }
    initIndexesFlags[elasticId] = true;
    ...
    for (const type of entityTypes) {
      promises.push(e.ensureType({type}));
    }
    yield Promise.all(promises);
    ...
    if (indexerMappingsByType && indexerMappingsByType.length > 0) {
      ...
      for (const mapping of indexerMappingsByType) {
        promises.push(e.putMapping(mapping));
      }
      yield Promise.all(promises);
    }
  } else {
