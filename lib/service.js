...
Goblin.registerQuest(goblinName, 'generate-workitems-templates', function(
  ...
  const tryPushFile = (filePath, collection) => {
  ...
  const browseCollections = collection => {
    for (const ref of Object.values(collection)) {
      ...
      const filePath = path.join(goblinRoot, `${type}-workitem.js`);
      tryPushFile(filePath, workitemFiles);

      if (common.referenceUseArity(ref)) {
        const filePath = path.join(goblinRoot, `${type}-plugin.js`);
        tryPushFile(filePath, pluginFiles);
      } else {
        const refConfig = getEntityConfig(type);
        if (refConfig.indexer) {
          const filePath = path.join(goblinRoot, `${type}-hinter.js`);
          tryPushFile(filePath, hinterFiles);
        }
      }
    }
  };
  ...
  const filePath = path.join(
    goblinRoot,
    `${rootEntityConfig.type}-workitem.js`
  );
  tryPushFile(filePath, workitemFiles);
  ...
  if (rootEntityConfig.indexer) {
    ...
    tryPushFile(filePath, searchFiles);
  }
