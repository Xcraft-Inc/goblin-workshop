...
const logicHandlers = {
  ...
  maintenance: (state, action) => {
    const enable = action.get('enable');
    if (typeof enable === 'boolean') {
      state = state
        .set('maintenance.enable', enable)
        .set('maintenance.message', enable ? action.get('message') : null);
    }
    const progress = action.get('progress');
    if (progress !== undefined && progress !== null) {
      state = state.set('maintenance.progress', progress);
    }
    return state;
  },
};
...
Goblin.registerQuest(goblinName, '_ripley', function*(
  ...
  quest.defer(() => {
    quest.warehouse.maintenance(
      ...
      () => {
        quest.me.maintenance({enable: false, progress: 0});
  ...
  quest.me.maintenance({enable: true, description: message});
