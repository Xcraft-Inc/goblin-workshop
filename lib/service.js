...
const logicHandlers = {
  ...
  'maintenance-enable': (state, action) =>
    state.set('maintenance.enable', action.get('enable')),
  'maintenance-progress': (state, action) =>
    state
      .set('maintenance.progress.position', action.get('position'))
      .set('maintenance.progress.length', action.get('length')),
};
...
Goblin.registerQuest(goblinName, '_ripley', function*(
  ...
  quest.defer(() => {
    quest.warehouse.maintenance(
      ...
      () => {
        quest.dispatch('maintenance-enable', {enable: false});
  ...
  quest.dispatch('maintenance-enable', {enable: true});
  ...
  quest.defer(
    quest.sub(`cryo.thawed.${dbSrc}`, function*(err, entry) {
      ...
      quest.dispatch('maintenance-progress', {progress});
  ...
  if (dbSrc !== dbDst || timestamp < stats.timestamp) {
    // TODO: create a store with the new current actions
  }
