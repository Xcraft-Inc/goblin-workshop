...
Goblin.registerQuest(goblinName, 'init', function*(quest) {
  ...
  quest.do({available: isUsable, configurations});
...
Goblin.registerQuest(goblinName, 'prepare-ripley', function*(
  ...
  try {
    ...
    yield quest.me.startRipley({configurator, dbSrc, dbDst, timestamp});
  } finally {
...
Goblin.registerQuest(goblinName, 'start-ripley', function*(
  ...
  if (configurator) {
    configuration = {
      rethinkdbHost: configurator.rethinkdbHost,
      elasticsearchUrl: configurator.elasticsearchUrl,
    };
  } else {
  ...
  yield poly.bootDatabases({configuration, desktopId});
  ...
  quest.defer(
    quest.sub(`cryo.thawed.${dbSrc}`, function(err, {msg}) {
