...
Goblin.registerQuest(
  goblinName,
  'subscribe',
  function*(quest, goblinId, feeds) {
    const subs = quest.goblin.getX('subs');
    subs[goblinId] = quest.sub(`*::${goblinId}.loadgraph-requested`, function*(
      err,
      msg
    ) {
      msg.data.$orcName = msg.orcName;
      yield quest.me.load({...msg.data});
    });
    yield quest.warehouse.attachToParents({
      branch: goblinId,
