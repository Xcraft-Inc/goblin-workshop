...
function checkEntity(entity, reporter, types, options) {
  ...
  return {
    entityId: entity.get('id'),
    rootAggregateId: entity.get('meta')
      ? entity.get('meta').get('rootAggregateId')
      : null,
    rootAggregatePath: entity.get('meta')
      ? entity.get('meta').get('rootAggregatePath').toArray()
      : null,
    entityFixPatch,
    propsToRemove,
    pointerToRemove,
...
function checkProperty(entityId, propertyName, propertyInfo, value) {
  ...
  //wrap when needed
  if (typeof value === 'object' && !Goblin.Shredder.isShredder(value)) {
