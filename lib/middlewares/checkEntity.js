...
function writeBadValue(reporter, entityId, prop, type, value) {
  writeLine(
    reporter,
    'bad-value',
  ...
  switch (conf.type) {
    case 'string':
      if (typeof value !== 'string') {
        writeBadValue(reporter, entityId, prop, conf.type, value);
      }
      break;
    case 'bool':
      if (typeof value === 'string') {
        ...
        if (value === 'false' || value === 'true') {
          writeLine(
            reporter,
            'warning',
            `${entityId}[${prop}] (type='${conf.type}' value='${value}') Value must be a native boolean`
          );
        } else {
          writeBadValue(reporter, entityId, prop, conf.type, value);
        }
