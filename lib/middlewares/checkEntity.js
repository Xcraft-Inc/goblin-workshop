...
module.exports = function(
  ...
  for (const prop of Object.keys(entity)) {
    ...
    if (configuration.properties[prop] === undefined) {
      writeLine(
        reporter,
        'missing-in-properties-schema',
        entity.id,
        prop,
        `Property ${prop} is not in the properties schema`
      );
  ...
  if (entity.meta.summaries) {
    for (const prop of Object.keys(entity.meta.summaries)) {
      if (configuration['summaries'][prop] === undefined) {
        writeLine(
          reporter,
          'missing-in-summaries-schema',
          entity.id,
          `summaries.${prop}`,
          `Summarie ${prop} not described in summaries schema`
        );
        if (deleteMissingKeys) {
  ...
  if (entity.sums) {
    for (const prop of Object.keys(entity.sums)) {
      if (configuration['sums'][prop] === undefined) {
        writeLine(
          reporter,
          'missing-in-sums-schema',
          entity.id,
          `sums.${prop}`,
          `Sums ${prop} not described in sums schema`
        );
        if (deleteMissingKeys) {
