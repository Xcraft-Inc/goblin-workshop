...
function* checkType(
  ...
  // Check the undefined fields first. They are never accepted.
  if (value === undefined) {
    const fix = {
      from,
      topic,
  ...
  // Checks for missing fields that are required.
  if (conf.required && (value === null || value === '')) {
    const fix = {
      from,
      topic,
      entityId,
  ...
  // A field can contain multiple type sometimes
  if (pathType === 'any') {
    return;
  }

  // For historical reason, number fields can contains string. Convert it to native.
  if (pathType === 'number' && typeof value === 'string') {
    const fix = {
      from,
      topic,
      entityId,
      ...
      correction: parseFloat(value),
      message: 'String number detected (it is no longer accepted from now on)',
    };
    fixPath(
      reporter,
      types,
  ...
  // For historical reason, integer fields can contains string. Convert it to native.
  if (pathType === 'integer' && typeof value === 'string') {
    const fix = {
      from,
      topic,
      entityId,
      ...
      value,
      valueType: typeof value,
      correction: parseInt(value),
      message: 'String integer detected (it is no longer accepted from now on)',
    };
    fixPath(
      reporter,
      types,
  ...
  switch (pathType) {
    ...
    case 'string':
      if (isNotNull(value) && typeof value !== 'string') {
        let correction = conf.defaultValue;
        if (isList(value)) {
          correction = value.valueSeq().toArray().join(',');
    ...
    case 'enum':
      ...
      break;
    case 'nabu':
      if (!StringBuilder.isTranslatable(value)) {
        const fix = {
          from,
          topic,
          entityId,
      ...
      break;
    case 'translatable':
      if (isNotNull(value) && value !== conf.defaultValue) {
        const finalValue = isShredder(value) ? value.toJS() : value;
        const nabuId = `${entityId}.${path}`;
        if (
          typeof finalValue !== 'object' ||
          !finalValue.nabuId ||
          finalValue.nabuId !== nabuId
        ) {
          let fix = {};

          if (typeof finalValue === 'string') {
            if (options && options.includes('fix-value-fields')) {
              yield quest.cmd('nabu.add-translatable-data', {
                nabuId,
                desktopId: quest.getDesktop(),
                entityId,
                defaultTranslation: finalValue,
              });
            }
            ...
            fix = {
              ...
              correction: WrapT(nabuId, null, null, null, true),
              message:
                'String detected (used as default value for all translations)',
            };
          } else {
            fix = {
              from,
              ...
              valueType: typeof finalValue,
              correction: WrapT(nabuId, null, null, null, true),
              message: 'Bad translatable value',
            };
          }
  ...
  for (const [prop, info] of Object.entries(configuration[mode + 's'])) {
    ...
    if (isCollection) {
      ...
      for (const id of collection) {
        ...
        const existingType = id.split('@', 1)[0];
      ...
      if (reference) {
        ...
        const existingType = reference.split('@', 1)[0];
        if (existingType !== type) {
          const fix = {
            from,
            topic: `bad-pointer-${mode}`,
...
function* checkEntity(quest, entity, from, reporter, types, options) {
