...
function fixPath(
  ...
  if (options.includes(fieldOption)) {
...
function removePointer(reporter, options, fieldOption, fix, pointerToRemove) {
  if (options.includes(fieldOption)) {
    if (options.includes('check-undefined-schema-fields')) {
      report(reporter, fix);
    }
...
function checkType(
  ...
  const fieldOption = `field-${conf.type}`;
...
function checkPointers(
  ...
  for (const [prop, info] of Object.entries(configuration[mode])) {
    ...
    const fieldOption = `field-pointer-${mode}`;
    ...
    if (isCollection) {
      ...
      for (const id of collection) {
        if (id.indexOf('@') === -1) {
          ...
          removePointer(reporter, options, fieldOption, fix, pointerToRemove);
          continue;
        }
...
module.exports = function(entity, reporter, options) {
