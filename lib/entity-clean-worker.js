...
Goblin.registerQuest(goblinName, 'patch-entity', function*(
  ...
  try {
    ...
    yield entityAPI.apply({patch, force: true});
    yield entityAPI.persist();

    quest.log.verb(`Cleaner Worker: patching  ${id} [DONE]`);
  } catch (ex) {
    const err = `Cleaner Worker: error during patching, ${ex.stack ||
      ex.message ||
      ex}`;
    quest.log.err(err);
    throw new Error(err);
...
Goblin.registerQuest(goblinName, 'clean-entity', function*(
  ...
  try {
    ...
    const err = `Cleaner Worker: error during cleaning props, ${ex.stack ||
      ex.message ||
      ex}`;
    quest.log.err(err);
    throw new Error(err);
