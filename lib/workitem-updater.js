...
const entityIdMutex = locks.getMutex;
const subs = {};

Goblin.registerQuest(
  goblinName,
  'subscribe',
  function*(quest, entityId, workitemId) {
    if (!entityId) {
      return;
    }
    ...
    if (!subs[entityId]) {
      const unsub = quest.sub(`*::${entityId}.changed`, function*(err, msg) {
        const {id} = msg.data;
        if (subs[id]) {
          yield quest.me.updateWorkitems({entityId: id});
        }
      });
...
Goblin.registerQuest(goblinName, 'update-workitems', function*(
  ...
  try {
    if (subs[entityId]) {
      ...
      let workitemAPI = yield quest.create(id, {
        id,
        desktopId: null,
      });
