...
Goblin.registerQuest(goblinName, 'unsubscribe', function*(
  ...
  quest.defer(() => entityIdMutex.unlock(entityId));
  if (!subs[entityId]) {
    throw new Error(`Workitem updater: No subscribe done for ${entityId}`);
  } else {
...
Goblin.registerQuest(goblinName, 'update-workitems', function*(
  ...
  try {
    if (subs[entityId]) {
      ...
      // Get new buttons oneTime
      const newButtons = yield workitemAPI.getNewButtonsState();
      ...
      for (const api of toUpdateAPIs.values()) {
        api.updateButtons({newButtons}, next.parallel());
      }
      yield next.sync();
    }
  } catch (ex) {
