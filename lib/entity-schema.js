...
const getAllEntitiesByType = (r, entityType) => {
  return r.table(entityType).getAll();
};

const insertCleanedEntities = (r, entityType, cleanedEntities) => {
  r.table(entityType)
...
Goblin.registerQuest(goblinName, 'checkEntities', function*(quest, entityType) {
...
Goblin.registerQuest(goblinName, 'cleanEntities', function*(quest, entityType) {
  const r = quest.getStorage('rethink');
  const entities = yield r.query({
    query: getAllEntitiesByType,
    args: [entityType],
