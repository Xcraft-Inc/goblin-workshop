...
const logicHandlers = {
  ...
  sample: Goblin.Shredder.mutableReducer((state, action) => {
    ...
    const times = state.get(`private.channels.${channel}.times`);
    const newTimes = times.set(times.size - 1, time);

    state = state.set(`private.channels.${channel}.samples`, newSamples);
    state = state.set(`private.channels.${channel}.times`, newTimes);
