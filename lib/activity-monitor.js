...
function delayedProgress(delayedValue, value) {
  if (delayedValue > value) {
    return delayedValue - (delayedValue - value) * 0.1;
  } else {
    return delayedValue - (delayedValue - value) * 0.4;
  }
}
...
Goblin.registerQuest(goblinName, 'init', function*(quest) {
  ...
  quest.do();
  const unsub = quest.sub(`*::job-queue.sampled`, function*(err, {msg, resp}) {
