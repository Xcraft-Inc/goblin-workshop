...
Goblin.registerQuest(goblinName, 'init', function (quest) {
  ...
  const drillQueue = new JobQueue(
    ...
    100,
    {waitOn: ['workshop', 'business'], waitDelay: 100}
  );
  ...
  quest.goblin.defer(
    quest.sub.local('*::*.<drill-down-requested>', function (err, {msg, resp}) {
  ...
  quest.goblin.defer(
    quest.sub('*::*.cache-entities-requested', function (err, {msg, resp}) {
      ...
      cacheQueue.push({
        id: msg.id,
        work: {entities, desktopId, ttl, busToken: xBus.getToken()},
        resp,
      });
    })
  );
