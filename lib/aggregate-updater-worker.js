...
Goblin.registerQuest(goblinName, 'checkChanges', function* (
  quest,
  desktopId,
  changes
) {
  quest.log.verb(`Aggregate updater: checking changes...`);
  ...
  for (const change of changes) {
    ...
    switch (change.action) {
      case 'run': {
        const call = common.jsifyQuestName(change.quest);
        if (!entityAPI[call]) {
          throw new Error(
            `Failed to apply a change, cannot run quest ${change.quest}`
          );
        }
        break;
      }
      ...
      case 'clear': {
        ...
        switch (propKind) {
          ...
          case 'value':
          case 'reference': {
            // prop exist and it's a collection
            break;
          }
          default:
            throw new Error(
  ...
  quest.log.verb(`Aggregate updater: checking changes [DONE]`);
});
  ...
  for (const change of changes) {
    ...
    switch (change.action) {
      case 'run': {
        const call = common.jsifyQuestName(change.quest);
        if (entityAPI[call]) {
          yield entityAPI[call](change.payload || {});
        } else {
          throw new Error(
            `Failed to apply a change, cannot run quest ${change.quest}`
          );
