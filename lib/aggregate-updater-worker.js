  ...
  for (const change of changes) {
    ...
    switch (cAction) {
      ...
      case 'patch': {
        ...
        yield entityAPI.apply({patch: cPayload, noRehydrate: true});
        break;
      }
      case 'clear': {
        if (level === 0) {
        ...
        switch (propKind) {
          ...
          case 'reference': {
            ...
            yield entityAPI[call]({noRehydrate: true});
            break;
          }
          default:
            throw new Error(
      ...
      case 'add': {
        ...
        switch (propKind) {
          ...
          case 'value': {
            ...
            yield entityAPI[call]({payload, noRehydrate: true});
            break;
          }
          case 'reference': {
            if (level === 0) {
            ...
            if (entityId) {
              ...
              yield entityAPI[call]({entityId, noRehydrate: true});
            } else {
              const payload = cPayload || {};
              let entityId;
              if (payload.id) {
