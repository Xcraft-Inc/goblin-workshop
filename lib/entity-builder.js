...
function builder(config) {
  ...
  Goblin.registerQuest(goblinName, 're-hydrate', function(
    quest,
    muteChanged,
    muteHydrated,
    options
  ...
  Goblin.registerQuest(goblinName, 'apply', function*(
    ...
    quest.do();
    if (!sync) {
      yield quest.me.reHydrate({muteChanged, muteHydrated});
    } else {
  ...
  function isSpecial(summary) {
    ...
    return (
      ...
      (summary._type === 'markdownWithRefs' || summary.nabuId)
    );
  }
  // end temporary
  if (indexer) {
    Goblin.registerQuest(goblinName, 'index', function*(
      ...
      if (Object.keys(doc).some(key => isSpecial(doc[key]))) {
        // HACK: we still don't know how to perform multilanguage indexing in elasticsearch, therefore
        // if we need it, just skip it (temporary)
        //const multiLanguageDoc = yield buildMultiLanguageSummaries(quest, doc);
        return;
      }

      // TODO: change to multiLanguageDoc
