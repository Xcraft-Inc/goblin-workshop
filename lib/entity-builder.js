...
function builder(config) {
  ...
  Goblin.registerAliasNamespace(`${goblinName}-hinter`, 'hinter');
  Goblin.registerAliasNamespace(`${goblinName}-detail`, 'detail');
  ...
  Goblin.registerQuest(goblinName, 'load-graph', function* (
    ...
    if (level <= stopAtLevel) {
      ...
      if (!graphSubscribers[desktopId]) {
        ...
        graphSubscribers[desktopId].startUnsub = quest.sub.local(
          `*::${quest.goblin.id}.<collection-changed>`,
          handleCollectionChanged(quest, desktopId, skipped)
        );
        graphSubscribers[desktopId].stopUnsub = quest.sub.local(
