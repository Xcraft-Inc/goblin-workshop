...
function builder(config) {
  ...
  const notifyChanged = quest => {
    quest.log(`${quest.goblin.id} changed`);
    quest.evt('changed');
  };
    ...
    try {
      ...
      if (lastHydrate) {
        ...
        if (
          ...
          quest.log(`skip hydrate ${entity.get('id')}`);
          quest.evt('hydrated');
          return;
        }
      }
      quest.log(`hydrate ${entity.get('id')}`);
  ...
  Goblin.registerQuest(goblinName, 'persist', function(quest, ripley) {
    ...
    if (oldState && state.equals(oldState)) {
      quest.log(`skip persist ${state.get('id')}`);
      quest.evt('persisted');
      return;
    }
    quest.log(`persist ${state.get('id')}`);
