...
function builder(config) {
  ...
  Goblin.registerQuest(goblinName, 'after-create', function* (quest, next) {
      ...
      for (const path in references) {
        ...
        if (common.referenceUseArity(ref)) {
          for (const rId of entity.get(path).values()) {
            ...
            // RE-HYDRATE
            refSubs[rId].push(
              quest.sub(`*::${rId}.<entity-changed>`, hydrator)
            );
          }
        } else {
          // Entity case
  ...
  Goblin.registerQuest(goblinName, 'copy-collection-by-ref', function* (
    ...
    for (const entityId of entityIds) {
      ...
      refSubs[entityId].push(
        quest.sub(`*::${entityId}.<entity-changed>`, hydrator)
      );
  ...
  Goblin.registerQuest(goblinName, 'copy-collection-by-value', function* (
    ...
    for (const newEntityAPI of newEntityAPIs) {
      ...
      valSubs[newEntityId].push(
        quest.sub(`*::${newEntityId}.<entity-changed>`, hydrator)
      );
  ...
  Goblin.registerQuest(goblinName, 'set-val', function* (
    ...
    valSubs[entity.id].push(
      quest.sub(`*::${entity.id}.<entity-changed>`, hydrator)
    );
  ...
  Goblin.registerQuest(goblinName, 'set-new-val', function* (
    ...
    const api = yield quest.create(type, {
      id: newEntityId,
      desktopId,
      ...payload,
      parentEntity: quest.goblin.id,
  ...
  const buildEntityFlowPropagator = (verb, evt) => {
    ...
    common.registerQuests(goblinName, {
      [questName]: function* (quest, next) {
        ...
        quest.evt(
          `${document.get('id')}.${verb}-<entity-flow-change-requested>`,
          {
            desktopId: quest.getSystemDesktop(),
            verb,
            entity: document,
            ...
            for (const path of values.keys()) {
              ...
              if (entityIds) {
                if (typeof entityIds === 'string') {
                  const entity = subDoc.get(`private.${path}.${entityIds}`);
                  if (entity) {
                    ...
                    quest.evt(
                      `${entity.get(
                        'id'
                      )}.${verb}-<entity-flow-change-requested>`,
                      {
                        desktopId: quest.getSystemDesktop(),
                        verb,
                        entity,
