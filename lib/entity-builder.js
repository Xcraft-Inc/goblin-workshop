...
function builder(config) {
    ...
    function* (
      ...
      if (!entity) {
        ...
        /*if (isNew)          
          ...
          completesEntityWithDefaultValues(goblinName, entity);

          // Check new entity, just after onNew.
          const ok = yield* checkNewEntity(quest, goblinName, entity.toJS());
          if (!ok) {
  ...
  if (computer) {
    Goblin.registerQuest(goblinName, 'compute', function* (
      ...
      const ok = yield* checkSums(quest, goblinName, sums);
      if (!ok) {
