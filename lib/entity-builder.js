...
function builder(config) {
    ...
    function* (
      ...
      if (!rootAggregatePath) {
        ...
        if (rootAggregatePath.toArray) {
          rootAggregatePath = rootAggregatePath.toArray();
        }
      }
      ...
      let isTrashed = false;
      const r = storage
        ? quest.getAPI(storage)
        : quest.getStorage(entityStorage);
      if (storage) {
        quest.log.dbg(`${id} using storage ${storage}`);
      }
      // Copy case init:
      if (copyId) {
  ...
  Goblin.registerQuest(goblinName, 'load-graph', function* (
    ...
    if (level <= stopAtLevel) {
      ...
      if (toCreate.length) {
        ...
        entitiesAPI = yield next.sync();
      }

      if (entitiesAPI && entitiesAPI.length) {
  ...
  Goblin.registerQuest(goblinName, 'rebuild', function* (quest) {
    //rebuild values
    const storage = quest.goblin.getX('storage');
    const r = storage ? quest.getAPI(storage) : quest.getStorage(entityStorage);
    const toCheck = {};
    const entity = quest.goblin.getState();
  ...
  Goblin.registerQuest(
    ...
    function* (quest, path, entity, remote, beforeId) {
      ...
      if (
        entity.get('meta.rootAggregatePath').toArray().join('/') !==
  ...
  if (buildSummaries) {
    Goblin.registerQuest(goblinName, 'build-summaries', function* (
      ...
      const ok = yield* checkSummaries(quest, goblinName, summaries);
      if (!ok) {
  ...
  if (computer) {
    Goblin.registerQuest(goblinName, 'compute', function* (
        ...
        Object.keys(peers)
          ...
          .forEach((type) => {
            ...
            Object.keys(computerState).forEach((sum) => {
              ...
              if (!computerState[sum]) {
                computerState[sum] = new BigNumber(0);
              }
      ...
      const ok = yield* checkSums(quest, goblinName, computerState);
      if (!ok) {
        //- throw new Error(`Fatal error, inconsistant sums in entity id=${entity.id}`);
      }
  ...
  Goblin.registerQuest(goblinName, 'hard-delete-entity', function* (
    ...
    if (indexer) {
      console.log('unindexing ', entity.id);
    ...
    const subDelete = watt(function* (subEntity) {
      for (const path in subEntity.meta.values) {
        for (const entityId of subEntity[path]) {
          ...
          if (toDelete) {
            const subType = toDelete.id.split('@')[0];
            ...
            if (configs[subType].indexer) {
              console.log('unindexing ', toDelete.id);
              yield e.unindex({
                type: subType,
                documentId: toDelete.id,
              });
