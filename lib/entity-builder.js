...
const handleCollectionChanged = (quest, desktopId, skipped) => (err, msg) => {
    ...
    switch (eventType) {
      case 'added':
        quest.create(entityId, {
          ...
          desktopId,
          mustExist: true,
          rootAggregateId: quest.goblin.getState().get('meta.rootAggregateId'),
...
function builder(config) {
  ...
  Goblin.registerQuest(goblinName, 'create', function*(
    ...
    if (!entity) {
      ...
      if (rootAggregateId === id) {
        entity = yield r.get({table: type, documentId: id, privateState: true});
      } else {
        const rootType = rootAggregateId.split('@')[0];
  ...
  Goblin.registerQuest(goblinName, 'change', function*(quest, path, newValue) {
    ...
    if (entity.get('meta.status') === 'archived') {
      console.warning(
        'Trying to mutate an archived document ',
        quest.goblin.id
      );
      return;
    }
