...
function builder(config) {
  ...
  Goblin.registerQuest(
    goblinName,
    'set-new-val',
    function* (quest, path, type, payload) {
      const state = quest.goblin.getState();
  ...
  if (indexer) {
    ...
    Goblin.registerQuest(goblinName, 'set-index', function* (
      ...
      const res = yield e.bulk({body, withInfo: true});
      if (res.created > 0 || res.updated > 0 || res.deleted > 0) {
        //will trigger to much side effects with search list
        //quiet mode must be handled before emitting
        //quest.evt(`${type}-index-changed`);
      }
    });
  }
