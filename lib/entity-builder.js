...
function builder(config) {
  ...
  if (references) {
    const refQuests = buildReferencesQuests(references, notifyChanged);
    common.registerQuests(goblinName, refQuests);
  }

  if (values) {
    const valQuests = buildValuesQuests(values, notifyChanged);
    common.registerQuests(goblinName, valQuests);
  }

  if (actions) {
    Object.assign(logicHandlers, actions);
    common.registerActions(goblinName, actions, notifyChanged);
  }
  ...
  const notifyChangedAndEmit = watt(function*(quest, verb, payload) {
    quest.evt(verb, payload);
    yield quest.me.notifyChanged();
  });

  const requestHydrate = quest => () => {
          ...
          if (changed) {
            if (entity.get('meta.rootAggregateId') !== quest.goblin.id) {
              try {
                ...
                quest.log.warn(
                  `Cannot rehydrate parent entity, error during parent loading: ${ex.stack ||
                    ex}`
                );
              }
            }
  ...
  const entityFlowQuests = {
    publish: function*(quest) {
      ...
      yield notifyChangedAndEmit(quest, 'published', null);
    },
    archive: function*(quest) {
      ...
      yield notifyChangedAndEmit(quest, 'archived', null);
    },
    restore: function*(quest) {
      ...
      //rollback
      quest.do();
      yield notifyChangedAndEmit(quest, 'restored', null);
    },
  };
