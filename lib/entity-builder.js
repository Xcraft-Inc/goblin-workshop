...
function* addEntityNotification(quest, desktop, message) {
...
function* addFixNotification(quest, desktop, fix) {
  const rows = [];
  pushRow(rows, 'Entity', fix.entityName);
  pushRow(rows, 'Property', fix.path);
  pushRow(rows, 'Type', fix.pathType);
  pushRow(rows, 'Value', fix.value);
  pushRow(rows, 'ValueType', fix.valueType);
  pushRow(rows, 'Message', fix.message);

  MD.flush();
  MD.addTitle(MD.bold('FATAL ERROR'));
...
function builder(config) {
  ...
  Goblin.registerQuest(goblinName, 'change', function* (quest, path, newValue) {
    ...
    if (entity.get('meta.status') === 'archived') {
      ...
      yield* addEntityNotification(quest, desktop, message);
      return;
    }
    ...
    if (newValue === undefined) {
      ...
      yield* addFixNotification(quest, desktop, fix);
      return;
    }
