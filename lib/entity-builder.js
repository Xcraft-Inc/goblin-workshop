...
function builder(config) {
  ...
  const notifyChangedAndEmitAfterPersist = watt(function*(
    quest,
    verb,
    payload,
    next
    ...
    const id = quest.goblin.getState().get('id');
    const unsubKey = `unsub-${quest.uuidV4()}`;
    quest.goblin.setX(
      unsubKey,
      quest.sub(`*::${id}.persisted`, () => {
        quest.goblin.getX(unsubKey)();
  ...
  const notifyChangedAndWaitPersist = watt(function*(quest, next) {
    const id = quest.goblin.getState().get('id');
    const unsubKey = `unsub-${quest.uuidV4()}`;
    quest.goblin.setX(
      unsubKey,
  ...
  Goblin.registerQuest(goblinName, 'create', function*(
    ...
    const notifyChangedWithDebounce = _.debounce(notifyChanged, 500);
    ...
    try {
      ...
      if (lastHydrate) {
        ...
        if (
          ...
          console.log(`skip hydrate ${entity.get('id')}`);
          quest.evt('hydrated');
          return;
        }
      }
      console.log(`hydrate ${entity.get('id')}`);
  ...
  Goblin.registerQuest(goblinName, 'change', function*(quest, path, newValue) {
    ...
    yield notifyChangedAndWaitPersist(quest);
  });
  ...
  Goblin.registerQuest(goblinName, 'move-ref', function*(
    ...
    quest.do();
    yield notifyChangedAndEmitAfterPersist(quest, 'plugin', {
      eventType: 'moved',
      entityId,
      beforeEntityId,
    });
  ...
  Goblin.registerQuest(goblinName, 'persist', function(quest, ripley) {
    ...
    if (oldState && state.equals(oldState)) {
      console.log(`skip persist ${state.get('id')}`);
      quest.evt('persisted');
      return;
    }
    console.log(`persist ${state.get('id')}`);
  ...
  const buildEntityFlowPropagator = (verb, evt) => {
    Goblin.registerQuest(goblinName, `${verb}-entity`, function*(quest, next) {
      ...
      console.log(`*::${document.get('id')}.${evt}`);
      quest.defer(
  ...
  Goblin.registerQuest(goblinName, 'publish', function*(quest) {
    ...
    notifyChangedAndEmitAfterPersist(quest, 'published');
  });
  ...
  Goblin.registerQuest(goblinName, 'restore', function(quest) {
    ...
    //rollback
    quest.do();
    notifyChangedAndEmitAfterPersist(quest, 'restored');
  });
  ...
  Goblin.registerQuest(goblinName, 'archive', function*(quest) {
    ...
    notifyChangedAndEmitAfterPersist(quest, 'archived');
  });
