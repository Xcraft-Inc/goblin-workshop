...
function builder(config) {
  ...
  Goblin.registerQuest(goblinName, 'after-create', function*(quest) {
    ...
    if (isNew) {
      if (afterNew) {
        yield quest.me.hydrate({persist: false});
        yield quest.me.afterNew({entity});
        quest.me.persist();
      } else {
        quest.me.hydrate({persist: true});
      }
    }
    quest.evt('loaded');
  });
  ...
  Goblin.registerQuest(goblinName, 'hydrate', function*(quest, persist, next) {
    try {
      let entity = quest.goblin.getState();
      let peers = null;
  ...
  Goblin.registerQuest(goblinName, 'update-aggregate', function(quest, entity) {
    quest.do();
    quest.me.hydrate({persist: true});
  });
  ...
  Goblin.registerQuest(goblinName, 'delete-aggregate', function(quest, entity) {
    quest.do();
  ...
  Goblin.registerQuest(goblinName, 'set-ref', function*(
    ...
    quest.me.hydrate({persist: !remote});
  });
