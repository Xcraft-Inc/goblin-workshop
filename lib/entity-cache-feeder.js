...
Goblin.registerQuest(goblinName, 'change-status', function*(
  ...
  const id = entity.get('id');
  quest.log.verb(`Entity Cache Feeder: changing status for ${id} ...`);
  try {
    const entityAPI = yield quest.create(id, {
      id: id,
      desktopId: desktopId,
    ...
    yield entityAPI.hydrate();
    yield entityAPI.persist();
    quest.log.verb(`Entity Cache Feeder: changing status ${id} [DONE]`);
  } catch (ex) {
    quest.log.err(
      `Entity Cache Feeder: error during status change, ${ex.stack ||
        ex.message ||
        ex}`
    );
  } finally {
