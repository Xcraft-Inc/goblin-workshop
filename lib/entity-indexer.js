...
Goblin.registerQuest(goblinName, 'init', function (quest) {
  console.log('\x1b[32m%s\x1b[0m', 'Goblin-Workshop: Entity Indexer [RUNNING]');
  ...
  const collector = new ArrayCollector(quest.newResponse(), 1000, onCollect);
  quest.goblin.defer(
  ...
  const evtDebouncer = new EventDebouncer(quest.newResponse(), 1000);
...
Goblin.registerQuest(goblinName, 'bulk', function* (quest, desktopId, body) {
  ...
  for (const [type, res] of Object.entries(infoByType)) {
    ...
    if (created > 0 || deleted > 0 || updated > 0) {
      evtDebouncer.publish(`${type}-<typed-index-changed>`);
    }
  }
});
