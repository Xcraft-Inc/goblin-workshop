...
function* checkNewEntity(quest, goblinName, entity) {
  ...
  if (data.length > 0) {
    ...
    const message = `The entity '${goblinName}' is inconsistent. The ${data.length} notifications below explain the issues.`;
    yield* addEntityNotification(quest, message);
...
function checkSummaries(quest, summaries) {
  // TODO
  return true; // ok
}
