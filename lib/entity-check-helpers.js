...
function* checkProperty(quest, entityName, propName, propInfo, newValue) {
  if (propInfo) {
    if (propInfo.type) {
...
function completesEntityWithDefaultValues(entityName, entity) {
  checkEntity.completesEntityWithDefaultValues(entityName, entity);
}

/******************************************************************************/

function* checkNewEntity(quest, entityName, entity) {
  const data = [];
...
function* checkSummaries(quest, entityName, summaries) {
  const entity = {
    id: entityName,
    meta: {summaries},
  };
  ...
  checkEntity.checkEntity(entity, reporter, null, ['check-only-meta']);
...
function* checkSums(quest, entityName, sums) {
  const entity = {
    id: entityName,
    sums,
  };
  ...
  checkEntity.checkEntity(entity, reporter, null, ['check-only-sums']);
