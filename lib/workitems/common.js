...
const registerActions = (goblinName, actions, notifyChanged) => {
  if (actions) {
    ...
    Object.keys(quests).forEach(q => {
      ...
      Goblin.registerQuest(goblinName, q, function*(quest, $msg, next) {
        ...
        const payload = Object.assign(
          ...
          {
            data: params.reduce((runParams, p, index) => {
              if (args[index]) {
        ...
        quest.evt('run-quest-requested', payload);
        const result = yield quest.sub.wait(`*::quest-runner.${runId}.done`);
        return result.data;
      });
