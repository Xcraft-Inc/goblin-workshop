...
const {string, js, reflect} = require('xcraft-core-utils');
    ...
    Object.keys(quests).forEach((q) => {
      Goblin.registerQuest(goblinName, q, function* (quest, $msg, next) {
        ...
        try {
          ...
          if (js.isGenerator(quests[q])) {
            return yield* quests[q](quest, ...questArguments, next);
          } else {
            return quests[q](quest, ...questArguments, next);
          }
        } finally {
          yield quest.sysKill();
        }
