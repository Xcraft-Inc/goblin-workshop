  ...
  if (actions) {
    Object.keys(actions).forEach((a) => {
      Goblin.registerQuest(goblinName, a, function* (quest) {
        ...
        const isClean = yield* checkNewEntity(quest, goblinName, entity);
        if (!isClean) {
...
const registerQuests = (goblinName, quests, subs) => {
  if (quests) {
    Object.keys(quests).forEach((q) => {
      Goblin.registerQuest(goblinName, q, quests[q], subs);
    });
  }
};
...
const getEntitiesQuest = function* (quest, type, entityIds) {
  ...
  fetched = fetched.map((entity) => new Goblin.Shredder(entity));
  return entities.concat(fetched);
};
...
const openWizard = function* (quest, name, form, kind, currentLocation) {
