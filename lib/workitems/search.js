...
module.exports = (config) => {
  ...
  Goblin.registerQuest(goblinName, 'setDetail', function* (quest, entityId) {
    ...
    const detailSet = quest.goblin.getX('desktopDetailIsSet');
    if (!detailSet) {
      const deskAPI = quest.getAPI(quest.getDesktop()).noThrow();
      yield deskAPI.setDetail({hinterId, workitemId: quest.goblin.id});
      quest.goblin.setX('desktopDetailIsSet', true);
    }
