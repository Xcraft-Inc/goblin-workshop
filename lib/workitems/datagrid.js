...
module.exports = config => {
  ...
  const changeListVisualization = watt(function*(quest, next) {
    const listId = quest.goblin.getX('listId');
    const listAPI = yield quest.getAPI(listId);
  ...
  Goblin.registerQuest(goblinName, 'toggle-sort', function*(
    ...
    quest.defer(function*() {
      yield changeListVisualization(quest, next);
      quest.me.loadMessages();
    });
  });
