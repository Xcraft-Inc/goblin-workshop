...
module.exports = config => {
  ...
  const methodBuilder = (verb, path) => {
    return jsifyQuestName(verb + '-' + (path ? path : type));
  };
  ...
  Goblin.registerQuest(goblinName, 'subscribe', function(quest) {
    ...
    const extractType = msg => msg.data.entityId.split('@')[0];

    const subscription = quest.sub(`*::${forEntity}.plugin`, (err, msg) => {
      ...
      switch (eventType) {
        ...
        case 'cleared':
          if (msg.data.type === type) {
            quest.me.clear({skipRemove: true});
          }
          break;
      }
    });
    quest.goblin.defer(subscription);
  ...
  Goblin.registerQuest(goblinName, 'add', function*(
    ...
    if (!entityId) {
      ...
      const rootAggregatePath = quest.goblin.getX('rootAggregatePath');
      const editor = yield quest.create(entityEditorId, {
        id: entityEditorId,
        desktopId,
        entityId: entityId,
        parentEntity: forEntityId,
    ...
    if (!skipAdd) {
      ...
      if (onAdd) {
        ...
        //XXX: we must adapt hook, avoiding toJS
        const entity = (yield addedEntityAPI.get()).toJS();
