...
module.exports = (config) => {
  ...
  Goblin.registerQuest(goblinName, 'subscribe', function (quest, $msg) {
    ...
    const entityPath = quest.goblin.getX('entityPath');
    if (existing) {
      //Unsub
      existing();
    }

    const subscription = quest.sub(`*::*.<collection-changed>`, function* (
      err,
      {msg, resp},
      next
    ) {
      const {path, emittedFromEntity} = msg.data;
      if (emittedFromEntity !== forEntity || path !== entityPath) {
        return;
      }
  ...
  Goblin.registerQuest(goblinName, 'add', function* (
    ...
    const editorRootAggregatePath = rootAggregatePath.length
      ? rootAggregatePath.concat([entityId])
      : [];
    if (newEntity) {
      editor = yield quest.create(entityEditorId, {
        ...
        mode: mode,
        rootAggregateId,
        rootAggregatePath: editorRootAggregatePath,
    ...
    if (!skipAdd) {
      ...
      const method = methodBuilder('addTo', entityPath);
  ...
  Goblin.registerQuest(goblinName, 'remove', function* (
    ...
    if (!skipRemove) {
      ...
      const method = methodBuilder('removeFrom', entityPath);
  ...
  Goblin.registerQuest(goblinName, 'do-action', function* (quest, action) {
    ...
    const service = parentWorkitemId.split('@', 1)[0];
  ...
  Goblin.registerQuest(goblinName, 'edit', function* (quest, entityId, view) {
    ...
    let description = entity.get('meta.summaries.info');
    if (Goblin.Shredder.isShredder(description)) {
      quest.log.warn(
        `A Shredder is used in 'meta.summaries.info' of the entity ${entityId}; it's forbidden because it breaks the serializers`
      );
  ...
  Goblin.registerQuest(goblinName, 'drag', function* (quest, fromId, toId) {
    ...
    const entityPath = quest.goblin.getX('entityPath');
    const forEntityAPI = quest.getAPI(forEntityId);
    const method = methodBuilder('moveInto', entityPath);
