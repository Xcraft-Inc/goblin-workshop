...
module.exports = config => {
  ...
  ///////////////////////////EXPORTS//////////////////////////////////////
  const getExportFilePath = function*(quest, desktopId, query, ext) {
    const fileName = `${quest.goblin.id.split('@')[0]}-${query}.${ext}`;
  ...
  Goblin.registerQuest(goblinName, 'export-to-csv', function*(quest) {
    ...
    const filePath = yield* getExportFilePath(quest, desktopId, query, 'csv');
  ...
  Goblin.registerQuest(goblinName, 'export-to-json', function*(quest) {
    const desktopId = quest.getDesktop();
    const query = quest.goblin.getState().get('query');
    if (query === 'none') {
      return;
    ...
    const filePath = yield* getExportFilePath(quest, desktopId, query, 'json');
