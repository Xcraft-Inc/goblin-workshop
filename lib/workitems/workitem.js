...
const unsubscribeToEntityChange = quest => {
  const subs = quest.goblin.getX('entitySubs');
  if (subs) {
...
module.exports = config => {
  ...
  const baseQuests = {
    ...
    'unload-graph': function*(quest, next) {
      //Currently used for unsubscribe to enitity changes
      const desktopId = quest.goblin.getX('desktopId');
      const mode = quest.goblin.getX('mode');
      const entity = quest.goblin.getX('loadedEntity');
      if (!entity) {
      ...
      const unloadGraph = (items, skipped, peers) => {
        for (const v in items) {
          if (common.referenceUseArity(items[v])) {
            const type = common.getReferenceType(items[v]);
            if (skipped.includes(type)) {
            ...
            if (mode !== 'backend') {
              for (const e of peers[v]) {
                ...
                const editorAPI = quest.getAPI(entityEditorId);
                editorAPI.unsubscribeToEntity({}, next.parallel());
              }
              continue;
            }
          } else if (entity.get(v) !== null) {
            ...
            if (mode !== 'backend') {
              ...
              if (editorId !== quest.goblin.id) {
                const editorAPI = quest.getAPI(editorId);
                editorAPI.unsubscribeToEntity({}, next.parallel());
              }
            }
          }
      ...
      if (level <= stopAtLevel) {
        const peers = yield buildPeers(quest, entity);
        if (entity.get('meta.references')) {
          unloadGraph(entity.get('meta.references').toJS(), skipped, peers);
        }

        if (entity.get('meta.values')) {
          unloadGraph(entity.get('meta.values').toJS(), skipped, peers);
        }
    ...
    'change-entity': function*(
      ...
      quest.evt('loaded');
      let res = yield quest.me.subscribeToEntity();
    ...
    'subscribe-to-entity': function(quest) {
      unsubscribeToEntityChange(quest);
      const entity = quest.goblin.getX('loadedEntity');
      if (!entity) {
        return quest.cancel();
      }
      const mode = quest.goblin.getX('mode');
      const _updater = (err, msg) => {
        if (!mode || mode === 'readonly' || enable['buttons']) {
      ...
      const updater = _.debounce(_updater, 200);

      //Subscribe
      const entitySubs = quest.goblin.getX('entitySubs');
      entitySubs.push(quest.sub(`${entity.get('id')}.changed`, updater));
