...
module.exports = config => {
  ...
  Goblin.registerQuest(goblinName, 'create', function*(
    ...
    console.log('LOADING ', entityId, ' AT LVL ', level);
    ...
    yield quest.me.loadGraph({level});
    return quest.goblin.id;
  });

  Goblin.registerQuest(goblinName, 'load-graph', function*(quest, level, next) {
    const desktopId = quest.goblin.getX('desktopId');
    const mode = quest.goblin.getX('mode');
    const entity = quest.goblin.getX('loadedEntity');
    const created = [];
    const loadGraph = (items, isVal, lvl) => {
      if (lvl > maxLevel) {
        return;
      }
    ...
    if (entity.get('meta.references')) {
      loadGraph(entity.get('meta.references').toJS(), false, level);
    }

    if (entity.get('meta.values')) {
      loadGraph(entity.get('meta.values').toJS(), true, level);
    }
  ...
  Goblin.registerQuest(goblinName, 'subscribe-to-entity', function(quest) {
    ...
    if (mainButton && (!mode || enable['mainButton'])) {
      entitySubs.push(
