...
module.exports = config => {
  ...
  const baseQuests = {
    ...
    'after-load': function*(quest) {
      ...
      if (quest.goblin.getX('level') === 1) {
        if (!mode || mode === 'readonly' || enable['buttons']) {
          const newButtons = yield quest.me.getNewButtonsState();
          yield quest.me.updateButtons({newButtons});
        }
      }
    },
    ...
    'get-new-buttons-state': function*(quest) {
    ...
    'update-buttons': function(quest, newButtons) {
