...
module.exports = config => {
  ...
  Goblin.registerQuest(goblinName, 'create', function*(
    ...
    quest.goblin.setX('createdByGraph', []);
  ...
  const baseQuests = {
    ...
    'load-graph': function*(quest, next) {
      const desktopId = quest.goblin.getX('desktopId');
      const mode = quest.goblin.getX('mode');
      ...
      if (level <= stopAtLevel) {
        ...
        quest.goblin.setX(
          'createdByGraph',
          quest.goblin.getX('createdByGraph').concat(created)
        );
      }
    },
    ...
    'change-entity': function*(
      ...
      const toKill = quest.goblin
        .getX('createdByGraph')
        .filter(c => c.entityId !== entityId);

      console.log('KILLING:', toKill.length);
      yield quest.kill(toKill.map(c => c.goblinId));
      const newCreatedByGraph = quest.goblin
        .getX('createdByGraph')
        .filter(c => c.entityId === entityId);
      quest.goblin.setX('createdByGraph', newCreatedByGraph);
