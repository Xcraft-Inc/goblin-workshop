...
module.exports = (config) => {
  ...
  Goblin.registerQuest(goblinName, 'create', function* (
          ...
          for (const handler of Object.keys(gadgets[key].onActions)) {
            quest.goblin.defer(
              quest.sub(`${newGadgetId}.${handler}`, function* (
                ...
                yield resp.cmd(`${goblinName}.${questName}`, msg.data);
              })
            );
          }
        }
    ...
    yield next.sync();
    yield quest.me.afterCreate({desktopId});
    quest.log.dbg(
  ...
  const baseQuests = {
    'after-create': function* (quest, desktopId) {
      const {
        entityId,
        parentEntity,
        rootAggregateId,
      ...
      yield quest.me.loadGraph({desktopId});
    },
    'load-graph': function* (quest, recycle, desktopId, next) {
      if (!desktopId) {
    ...
    'after-load': function* (quest) {
      ...
      yield quest.me.loadSettings();
    },
    'load-settings': function* (quest) {
      //load goblin settings for this workitem
      const entityId = quest.goblin.getX('entityId');
      const desktopId = quest.goblin.getX('desktopId');
      if (!entityId.startsWith('workitem@')) {
        quest.log.dbg(`Loading workitem fields for ${goblinName}...`);
        const workitemAPI = yield quest.create(`workitem@${goblinName}`, {
          id: `workitem@${goblinName}`,
          desktopId,
          name: goblinName,
        });
        ...
        quest.log.dbg(`Loading workitem fields ${goblinName} [DONE]`);
      }
    },
    ...
    'change-entity': function* (
      ...
      quest.evt('loaded');
      yield quest.me.afterLoad();
      yield quest.me.loadGraph({recycle: true, desktopId});
    ...
    'edit': function* (quest, entity, desktopId, currentLocation) {
      const desk = quest.getAPI(desktopId);
    ...
    'hide': function* (quest, currentLocation) {
      const deskAPI = quest.getAPI(quest.getDesktop());
      currentLocation = currentLocation.set('hash', '');
      yield deskAPI.updateCurrentLocation({currentLocation});
    },
    'close': function* (quest, kind, desktopId, contextId, currentLocation) {
  ...
  const entityFlowQuests = {
    'trash-entity': function* (quest, currentLocation) {
      let cancelToken = null;
      if (onTrash) {
        cancelToken = yield quest.me.onTrash({currentLocation});
      }
      if (quest.isCanceled(cancelToken)) {
        return cancelToken;
      }
    ...
    'archive-entity': function* (quest, currentLocation) {
      const mode = quest.goblin.getX('mode');
      let cancelToken = null;
      if (onArchive && (!mode || enable['onArchive'])) {
        cancelToken = yield quest.me.onArchive({currentLocation});
      }
      if (quest.isCanceled(cancelToken)) {
        return cancelToken;
      }
    ...
    'publish-entity': function* (quest, currentLocation) {
      const mode = quest.goblin.getX('mode');
      let cancelToken = null;
      if (onPublish && (!mode || enable['onPublish'])) {
        cancelToken = yield quest.me.onPublish({currentLocation});
      }
      if (quest.isCanceled(cancelToken)) {
        return cancelToken;
      }
    ...
    'submit-entity': function* (quest, currentLocation) {
      const mode = quest.goblin.getX('mode');
      let cancelToken = null;
      if (onSubmit && (!mode || enable['onPublish'])) {
        cancelToken = yield quest.me.onSubmit({currentLocation});
      }
      if (quest.isCanceled(cancelToken)) {
        return cancelToken;
      }
    ...
    'restore-entity': function* (quest, currentLocation) {
      const mode = quest.goblin.getX('mode');
      let cancelToken = null;
      if (onRestore && (!mode || enable['onRestore'])) {
        cancelToken = yield quest.me.onRestore({currentLocation});
      }
      if (quest.isCanceled(cancelToken)) {
        return cancelToken;
      }
