...
module.exports = (config) => {
  ...
  if (hinters) {
    for (const [namespace, def] of Object.entries(hinters)) {
      Goblin.registerAliasNamespace(`${namespace}-hinter`, `hinter`);
      if (def.hinter) {
        Goblin.registerAliasNamespace(
          `${def.hinter}-finder`,
          `${namespace}-hinter`
        );
      } else {
  ...
  Goblin.registerQuest(goblinName, 'create', function* (
    ...
    if (mode !== 'readonly' && mode !== 'backend') {
      ...
      if (onEntityPropertyChanged) {
        yield quest.me.listenEntityProperties({entityId});
      }
    }
  ...
  Goblin.registerQuest(goblinName, 'listenEntityProperties', function (
    quest,
    entityId
  ) {
    const goblinId = quest.goblin.id;
    quest.goblin.defer(
      quest.sub(`*::${entityId}.<entity-property-changed>`, function* (
        _,
        {msg, resp}
      ) {
        const {path, newValue} = msg.data;
        const quest = onEntityPropertyChanged[path];
        if (quest) {
          yield resp.cmd(`${goblinName}.${quest}`, {id: goblinId, newValue});
        }
      })
    );
  });
  ...
  const loadGraphInternal = watt(function* (quest, recycle, desktopId, next) {
    ...
    if (level <= stopAtLevel) {
      ...
      try {
        ...
        quest.log.dbg(`FAILED graph loading of ${quest.goblin.id}`);
      }
    }
