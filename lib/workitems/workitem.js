...
module.exports = config => {
  ...
  Goblin.registerQuest(
    ...
    function*(
      ...
      const workitemGraphLoaderAPI = quest.getAPI('graph-loader');
      workitemGraphLoaderAPI.subscribe(
        {goblinId: quest.goblin.id, feeds: Object.keys(quest.goblin.feed)},
        next.parallel()
      );
      yield next.sync();
      yield quest.me._$afterCreate();
      return quest.goblin.id;
    },
    ['*::*.loaded', '*::*.done'] //done is registered in case of possible wizard usage...
  );
  ...
  const baseQuests = {
    'after-create': function*(quest) {
      ...
      const createdEntity = yield quest.me._$createEntity({
        entityId,
        entity,
        parentEntity,
        rootAggregateId,
      ...
      if (level === 1 && mode !== 'backend') {
        yield quest.me._$subscribeToEntity();
      }
      quest.evt('loadgraph-requested', {
      ...
      if (level <= stopAtLevel) {
        if (mode === 'backend' || entityGraphOnly === true) {
          ...
          for (const c of toCreate) {
            quest.create(c.id, c.payload, next.parallel());
          }
  ...
  Goblin.registerQuest(goblinName, 'delete', function*(quest) {
    ...
    const workitemGraphLoaderAPI = quest.getAPI('graph-loader');
    yield workitemGraphLoaderAPI.unsubscribe({goblinId: quest.goblin.id});
  });
