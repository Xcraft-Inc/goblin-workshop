...
module.exports = (config) => {
  ...
  Goblin.registerQuest(goblinName, 'create', function* (
    ...
    if (!rootWorkitemId) {
      rootWorkitemId = quest.goblin.id;
    }
  ...
  const afterCreate = watt(function* (quest, desktopId) {
    ...
    yield loadGraphInternal(quest, false, desktopId);
  });
  ...
  const loadGraphInternal = watt(function* (quest, recycle, desktopId, next) {
    ...
    const {level, stopAtLevel, skipped, rootWorkitemId} = quest.goblin.getX(
      'loadGraphParams'
    );
  ...
  const baseQuests = {
    ...
    'change-entity': function* (
      ...
      quest.evt('<workitem-loaded>');
      yield quest.me.afterLoad();
      yield loadGraphInternal(quest, true, desktopId);
    ...
    'showHinter': function* (quest, type) {
      const deskAPI = quest.getAPI(quest.getDesktop());
      yield deskAPI.setHinter({
        workitemId: quest.goblin.getX('rootWorkitemId'),
        hinterId: `${type}-hinter@${quest.goblin.id}`,
      });
    },
    'hideHinter': function* (quest) {
      const deskAPI = quest.getAPI(quest.getDesktop());
      yield deskAPI.setHinter({
        workitemId: quest.goblin.getX('rootWorkitemId'),
        hinterId: null,
