...
const changePrivate = (
  ...
  entities = entities.reduce((entities, entity, newId) => {
    rootAggregatePathSuffix.push(newId);
    const rootAggregatePath = rootAggregatePathSuffix;
    entities = entities.set(
      newId,
      changeEntity(entity, newId, rootAggregateId, rootAggregatePath)
    );
    return entities;
  }, new Shredder({}));
  return entities;
...
const changeEntity = (entity, newId, rootAggregateId, rootAggregatePath) => {
  if (newId) {
...
module.exports = (entity, newId, rootAggregateId, rootAggregatePath) =>
  changeEntity(entity, newId, rootAggregateId, rootAggregatePath);
