...
const buildReferencesQuests = (references, rehydrate) => {
  ...
  for (const path in references) {
    ...
    if (common.referenceUseArity(ref)) {
      ...
      refQuests[`add-to-${path}`] = function* (
        quest,
        entityId,
        beforeId,
        noRehydrate,
      ...
      refQuests[`remove-from-${path}`] = function* (
        quest,
        entityId,
        noRehydrate,
        asyncHydrate
      ) {
        yield quest.sys.removeRef({path, entityId, noRehydrate, asyncHydrate});
      };
  ...
  for (const path in values) {
    ...
    if (common.referenceUseArity(val)) {
      ...
      valQuests[`add-to-${path}`] = function* (
        ...
        if (entity) {
          entity = new Goblin.Shredder(entity);
          yield quest.sys.addVal({path, entity, beforeId, asyncHydrate});
        } else {
          if (!entityId) {
            throw new Error(
              'Cannot add value ',
      ...
      valQuests[`clear-${path}`] = function* (quest, noRehydrate) {
